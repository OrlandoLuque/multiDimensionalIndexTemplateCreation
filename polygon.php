...
class Polygon {
    ...
    static function vertexIntsLine(&$v, &$q1, &$q2) {
        ...
        if ($x1 == $x2) {
            return $xv == $x1 && ($yv >= $y1 && $yv <= $y2 || $yv >= $y2 && $yv <= $y1);
        }
            ...
            do {
                ...
                do {
                    ...
                    if ($first) {
                        ...
                        $this->x_min = min($this->x_min, $v->X());
                        $this->x_max = max($this->x_max, $v->X());
                        $this->y_min = min($this->y_min, $v->Y());
                        $this->y_max = max($this->y_max, $v->Y());
                    }
                    if ($v->d() != 0) {
                        $v->setXc($v->Xc() * $sx);
                        $v->setYc($v->Yc() * $sy);
